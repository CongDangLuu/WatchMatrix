...
void EEPROMClass::readPtr(int const address, void *ptr, uint16_t size) {
  if (address < 0 || (size_t)address >= _size) {
    DEBUGV("EEPROMClass::read error, address %d > %d or %d < 0\n", address, _size, address);
    return;
  }
  ...
  memcpy(pdata,_data + address ,size);
...
void EEPROMClass::writePtr(int const address, void *ptr, uint16_t size) {
  if (address < 0 || (size_t)address >= _size) {
    DEBUGV("EEPROMClass::write error, address %d > %d or %d < 0\n", address, _size, address);
    return;
  }
  ...
  uint8_t *pdata = ( uint8_t *)ptr;
  if (memcmp(_data + address, pdata ,size) != 0) {
    _dirty = true;
    memcpy(_data + address, pdata ,size);
  }
}
...
uint8_t * EEPROMClass::getDataPtrAdd(uint16_t add) {
  // _dirty = true;
  return &_data[add];
}
